...
def test_root_port_link_speed_width():
    try:
        ...
        link_speed_path = os.path.join(device_path, "max_link_speed")
        with open(link_speed_path, "r") as f:
            link_speed = f.read().strip()
            print(f"Root Port Maximum Link Speed: {link_speed}")

        # Retrieve Current Link Width
        link_width_path = os.path.join(device_path, "max_link_width")
        with open(link_width_path, "r") as f:
            link_width = f.read().strip()
            print(f"Root Port Maximum Link Width: {link_width}")
